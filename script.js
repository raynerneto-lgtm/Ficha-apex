...
function updateBodyLimit(){
  const metabolismo = Number(document.querySelector('[data-key="metabolismo"]')?.value) || 0;
  const forca = Number(document.querySelector('[data-key="forca"]')?.value) || 0;
  const folego = Number(document.querySelector('[data-key="folego"]')?.value) || 0;
  const output = document.getElementById("limiteCorporalValue");
  if (!output) return;
  if (!folego) { output.textContent = "—"; return; }
  const value = Math.round((metabolismo + forca) / folego);
...
// MÚLTIPLAS FICHAS (criar, salvar e alternar)
const STORAGE_KEY = "apex_ficha_v1"; // chave antiga, usada só para migração
const FICHAS_KEY = "apex_fichas_v1";
const fichaSelect = document.getElementById("fichaSelect");
const newFichaBtn = document.getElementById("newFichaBtn");
const deleteFichaBtn = document.getElementById("deleteFichaBtn");

function nextFichaId(){
  return "ficha_" + Date.now() + "_" + Math.floor(Math.random() * 1000);
}

function loadFichasStore(){
  try {
    const raw = JSON.parse(localStorage.getItem(FICHAS_KEY));
    if (raw && raw.fichas && Object.keys(raw.fichas).length) return raw;
  } catch(e) {}

  // Migração: se existir uma ficha salva no formato antigo, ela vira a primeira ficha.
  const fichas = {};
  const id = nextFichaId();
  let name = "Ficha 1";
  let data = {};
  ...
  if (old) {
    try {
      data = JSON.parse(old) || {};
      if (data.nome && data.nome.trim()) name = data.nome.trim();
    } catch(e) {}
  }
  fichas[id] = { name, data };
  return { activeId: id, fichas };
}

let fichasStore = loadFichasStore();

function saveFichasStore(){
  localStorage.setItem(FICHAS_KEY, JSON.stringify(fichasStore));
}

function renderFichaSelect(){
  if (!fichaSelect) return;
  fichaSelect.innerHTML = "";
  Object.entries(fichasStore.fichas).forEach(([id, ficha]) => {
    const opt = document.createElement("option");
    opt.value = id;
    opt.textContent = ficha.name || "Sem nome";
    if (id === fichasStore.activeId) opt.selected = true;
    fichaSelect.appendChild(opt);
  });
}

function applyFichaData(data){
  load(data || {});
  initBonds();
  initInventory();
  updateImmunity();
...
function persistCurrentFicha(){
  const data = collect();
  const currentName = fichasStore.fichas[fichasStore.activeId]?.name || "Ficha sem nome";
  const name = (data.nome && data.nome.trim()) ? data.nome.trim() : currentName;
  fichasStore.fichas[fichasStore.activeId] = { name, data };
  saveFichasStore();
  return name;
}

function save(){
  const name = persistCurrentFicha();
  renderFichaSelect();
...
document.getElementById("clearBtn").addEventListener("click", () => {
  ...
  fichasStore.fichas[fichasStore.activeId] = { name: fichasStore.fichas[fichasStore.activeId]?.name || "Ficha sem nome", data: {} };
  saveFichasStore();
  applyFichaData({});
  document.getElementById("saveState").textContent = "FICHA LIMPA";
});

newFichaBtn?.addEventListener("click", () => {
  persistCurrentFicha();
  const name = (prompt("Nome da nova ficha:", "Nova Ficha") || "Nova Ficha").trim() || "Nova Ficha";
  const id = nextFichaId();
  fichasStore.fichas[id] = { name, data: {} };
  fichasStore.activeId = id;
  saveFichasStore();
  renderFichaSelect();
  applyFichaData({});
...
fichaSelect?.addEventListener("change", () => {
  const id = fichaSelect.value;
  if (!id || id === fichasStore.activeId) return;
  persistCurrentFicha();
  fichasStore.activeId = id;
  saveFichasStore();
  renderFichaSelect();
  applyFichaData(fichasStore.fichas[id]?.data || {});
  document.getElementById("saveState").textContent = "FICHA CARREGADA";
});

deleteFichaBtn?.addEventListener("click", () => {
  const ids = Object.keys(fichasStore.fichas);
  if (ids.length <= 1) { alert("Não é possível excluir a única ficha existente."); return; }
  const name = fichasStore.fichas[fichasStore.activeId]?.name || "esta ficha";
  if (!confirm(`Excluir "${name}" permanentemente?`)) return;
  delete fichasStore.fichas[fichasStore.activeId];
  fichasStore.activeId = Object.keys(fichasStore.fichas)[0];
  saveFichasStore();
  renderFichaSelect();
  applyFichaData(fichasStore.fichas[fichasStore.activeId]?.data || {});
  document.getElementById("saveState").textContent = "FICHA EXCLUÍDA";
});

renderFichaSelect();
load(fichasStore.fichas[fichasStore.activeId]?.data || {});
updateImmunity();
updateBodyLimit();
initResourceBars();
...
// FOTO DO PERSONAGEM (substitui o selo BIO HAZARD)
const portraitStamp = document.getElementById("portraitStamp");
...
const portraitDataEl = document.getElementById("portraitData");
const removePortraitBtn = document.getElementById("removePortraitBtn");

function updatePortrait(){
  const src = portraitDataEl?.value;
  if (src) {
    portraitImg.src = src;
    portraitImg.style.display = "block";
    portraitPlaceholder.style.display = "none";
    portraitStamp.classList.add("has-image");
  } else {
    portraitImg.removeAttribute("src");
    portraitImg.style.display = "none";
    portraitPlaceholder.style.display = "block";
    portraitStamp.classList.remove("has-image");
  }
}

portraitStamp?.addEventListener("click", (e) => {
  if (e.target === removePortraitBtn) return;
  portraitInput.click();
});
...
portraitInput?.addEventListener("change", () => {
  ...
  reader.onload = () => {
    portraitDataEl.value = reader.result;
    updatePortrait();
    document.getElementById("saveState").textContent = "ALTERAÇÕES NÃO SALVAS";
  };
...
removePortraitBtn?.addEventListener("click", (e) => {
  e.stopPropagation();
  portraitDataEl.value = "";
  updatePortrait();
  document.getElementById("saveState").textContent = "ALTERAÇÕES NÃO SALVAS";
});
